...
class Color {
 public:
  ...
  const unsigned char operator[](int index) const { return color[index]; }
  ...
  int getLuminance() const {
    return static_cast<int>(0.2126f * color[0] + 0.7152f * color[1] +
                            0.0722f * color[2]);
  }
  ...
  Color clamp(int min, int max) const {
    return Color(std::max(min, std::min(max, static_cast<int>(color[0]))),
  ...
  Color operator*(float f) const {
    return Color(static_cast<int>(color[0] * f), static_cast<int>(color[1] * f),
                 static_cast<int>(color[2] * f), static_cast<int>(color[3] * f))
        .clamp(0, 255);
  }
  ...
  Color operator+(const Color& c) const {
    return Color(color[0] + c.color[0], color[1] + c.color[1],
                 color[2] + c.color[2], color[3] + c.color[3])
        .clamp(0, 255);
  }
  ...
  Color operator+(float f) const {
    ...
    return Color(color[0] + val, color[1] + val, color[2] + val, color[3] + val)
        .clamp(0, 255);
  }
